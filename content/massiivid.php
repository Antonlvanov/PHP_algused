...
foreach ($colorsHTML as $colors) {
    echo "<span style='color: $colors'><br>$colors</span>";
    echo "<div style='background-color: $colors'>$colors</div>";
}
?>
</div>

</div>
<h2> <a href="https://www.metshein.com/unit/php-massiivid-ulesanne-5/">Ülesanne 5</a> </h2>
<div id="ul5">
    <strong>
    <h3> Autod – koosta auto markidest ja VIN koodideset massiivid </h3>
    <h4> Teosta järgmised protseduurid: </h4>
    ...
    * leia ja väljasta VIN koodid,  mille märkide arv on väiksem kui 17 <br>
    ...
    echo "<h3>Kõik auto markid: </h3>";
    echo "<ol>";
        foreach ($auto_markid as $auto) {
            echo "<li>$auto</li>";
        }
    echo "</ol>";
    ...
    echo "</ol>";
        ...
        echo "Autode arv: ".count($vin_numbers)." tk";

    echo "<h3>Kontrolli, kas massiviid on ühepikkused</h3>";
        if (count($auto_markid)==count($vin_numbers)) {
    ...
    echo "<h3>Leia ja väljasta VIN koodid, mille märkide arv on väiksem kui 17</h3>";
        foreach ($vin_numbers as $key=>$vin_n){
                if (strlen($vin_n) < 17) {
                    echo $auto_markid[$key]." - ".$vin_numbers[$key]."<br>"; }
            }
    ?>
</div>
<h2>Ülesanne 3.</h2>
<div id ="ul3">
    ...
    foreach ($pildid as $pilt) {
     if ($pilt != "." && $pilt != "..") {
         echo "<img src='$dir/$pilt' width='100px'><br>";
     }
    }
    ?>
</div>
